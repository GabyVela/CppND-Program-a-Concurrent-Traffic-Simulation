...
void TrafficLight::cycleThroughPhases()
{
  ...
  std::mt19937 mt(rd()); //A Mersenne Twister pseudo-random generator of 32-bit numbers with a state size of 19937 bits.
  ...
  while(true){
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(1));//sleep for 1ms between two cycles
    ...
    if (timeLapsed >= distr(mt))
    {
    	if (getCurrentPhase()==green){
        newCurrentPhase(red);
        }
      	else{
        newCurrentPhase(green);
